...
public class RSAUtils
{
  ...
  public static byte asc_to_bcd(byte asc) { byte bcd;
    byte bcd;
    if ((asc >= 48) && (asc <= 57)) {
      bcd = (byte)(asc - 48); } else { byte bcd;
      if ((asc >= 65) && (asc <= 70)) {
        bcd = (byte)(asc - 65 + 10); } else { byte bcd;
        if ((asc >= 97) && (asc <= 102)) {
          bcd = (byte)(asc - 97 + 10);
        } else
          bcd = (byte)(asc - 48); } }
...
/* Location:              E:\apache-tomcat-8\apache-tomcat-8\webapps\ROOT\WEB-INF\classes\!\org\takeback\util\RSAUtils.class
